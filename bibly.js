...
(function() {
	...
	var bibly = {
		...
		ver =  '((1?\\d{1,2})|'+rom+')([\.:]\\s?(\\d+))?(\\s?[-–&]\\s?(\\d+|'+rom+'))?',  // 1 OR 1:1 OR 1:1-2, 100, but not 1000
		ver2 =  '((1?\\d{1,2})|'+rom+')[\.:]\\s?(\\d+)(\\s?[-–&]\\s?(\\d+|'+rom+'))?',  // NOT 1 OR 1:1 OR 1:1-2 (this is needed so verses after semi-colons require a :. Problem John 3:16; 2 Cor 3:3 <-- the 2 will be a verse)		regexPattern = '\\b('+bok+')\.?\\s+('+ver+'((\\s?,\\s?'+ver+')|(\\s?;\\s?'+ver2+'))*)\\b',
		...
		parseRefText = function(refText) {
			...
			if (reference != null && typeof reference.isValid != 'undefined' && reference.isValid()) {
				...
				if (match) {				
					...
					p3 = parseInt(match[16],10);
					p5 = parseInt(match[18],10);
					...
					// 1:2
						...
						lastReference.verse1 = parseInt(match[16],10);
						lastReference.chapter2 = -1;
						lastReference.verse2 = -1;		
					...
					// 1:2-3
						...
						lastReference.verse1 = parseInt(match[16],10);
						lastReference.chapter2 = -1;
						lastReference.verse2 = parseInt(match[18],10);;		
